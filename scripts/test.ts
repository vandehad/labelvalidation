...
/* ---------- the print queue: what the relay sends and what it is called ---------- */
{
  ...
  {
    const said = describeRepeat(
      [
        { id: 1, kind: 'batch', status: 'done', labels: 1500, first_code: 'B0901A01', last_code: 'B3215D01' },
        { id: 2, kind: 'batch', status: 'queued', labels: 3720, first_code: 'B3215E01', last_code: 'B4634B01' },
      ],
      5220,
    )
    ok('a repeat says how many, where they went and which', said.includes('5,220 of these labels') && said.includes('1,500 already printed on the batch printer') && said.includes('3,720 already waiting for the batch printer') && said.includes('(B0901A01 to B4634B01)'))
  }
...
ok('seven bare digits are a one-digit zone, then two each', canonOld('7010101') === '07-01-01-01' && canonOld('1090305') === '01-09-03-05')
ok('and match the dashed and unpadded forms of the same bin', canonOld('7010101') === canonOld('07-01-01-01') && canonOld('7010101') === canonOld('7-1-1-1'))
ok('six bare digits are still ambiguous and left alone', canonOld('109035') === '109035')
...

{
  ...
  ok('seven digits are read outright now, so a listed one never reaches the suspects', wms.has(canonOld('1090305')))
  ok('six digits that split one way into the list are similar with that suggestion', s('109035').kind === 'similar' && s('109035').suggestions.join() === '01-09-03-05')
  ...
  ok('digits that split into nothing in the list are unknown', s('999999').kind === 'unknown')
  ok('seven digits are well-formed now, so an unlisted one is extra, not unknown', s('9999999').kind === 'extra')
